  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
        ...
        {Object.keys(tabContent).map((tab) => (
          <button
            ...
            className={`px-3 py-2 rounded-md text-sm font-medium ${
              activeTab === tab ? 'bg-blue-500 text-white' : 'text-gray-600 hover:bg-blue-100'
            }`}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        >
          <h1 className="text-4xl font-extrabold text-gray-900 text-center mb-8 mt-7">
          ...
          {features.map((feature, index) => (
            <motion.div
              ...
              className={`p-6 rounded-lg shadow-lg ${
                activeTab === feature.tab ? 'bg-blue-500 text-white' : 'bg-white'
              }`}
              whileHover={{ scale: 1.05 }}
              transition={{ type: 'spring', stiffness: 300 }}
              onClick={() => setActiveTab(feature.tab)}
            >
              <feature.icon className={`h-12 w-12 mb-4 ${activeTab === feature.tab ? 'text-white' : 'text-blue-500'}`} />
              <h3 className={`text-lg font-semibold mb-2 ${activeTab === feature.tab ? 'text-white' : 'text-gray-900'}`}>
                {feature.title}
              </h3>
              <p className={activeTab === feature.tab ? 'text-white' : 'text-gray-600'}>{feature.description}</p>
            </motion.div>
          ))}
        </motion.section>
        ...
        >
          <h2 className="text-2xl font-bold text-gray-900 mb-4">{tabContent[activeTab].title}</h2>
          <p className="text-gray-600">{tabContent[activeTab].content}</p>
        </motion.section>

        <section className="text-center">
          <h2 className="text-3xl font-bold text-gray-900 mb-8">Why Choose HealthHub?</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              >
                <benefit.icon className="h-12 w-12 text-blue-500 mx-auto mb-4" />
