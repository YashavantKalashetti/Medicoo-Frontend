...
const MedicationCard = ({ medication }) => {
  ...
  return (
    <Card className="overflow-hidden shadow-md transition-all duration-300 hover:shadow-xl border border-gray-200 rounded-lg">
      <CardContent className="p-0">
        <div className="p-6 bg-gradient-to-r from-white to-gray-50">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              <div className="w-12 h-12 rounded-full flex items-center justify-center mr-4" style={{ backgroundColor: medication.color }}>
                <Pill className="w-6 h-6 text-white" />
              </div>
              <div>
                <h3 className="text-2xl font-bold text-gray-900">{medication.medicine}</h3>
                <p className="text-sm font-medium text-gray-600">{medication.dosage}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-sm font-semibold text-gray-700">{daysLeft} days left</p>
              <p className="text-xs text-gray-500">of {medication.numberOfDays} days</p>
            </div>
          </div>
          <div className="space-y-3">
            <div className="flex items-center">
              <Clock className="w-5 h-5 mr-3 text-gray-400" />
              <span className="text-sm text-gray-700">{medication.instruction}</span>
            </div>
            <div className="flex items-center">
              <Calendar className="w-5 h-5 mr-3 text-gray-400" />
              <span className="text-sm text-gray-700">Valid till: {formattedDate}</span>
            </div>
            <div className="flex items-center">
              <AlertCircle className="w-5 h-5 mr-3 text-gray-400" />
              <span className="text-sm text-gray-700">Take {medication.numberOfTimes}x daily</span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold fdx ">Your Medications</h1>
        <Button onClick={() => setShowAddModal(true)} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-full shadow-md transition duration-300 ease-in-out transform hover:-translate-y-1">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg w-96 max-w-md shadow-lg">
            <div className="flex justify-between items-center mb-6">
              ...
              <Button onClick={() => setShowAddModal(false)} variant="ghost" className="p-1 hover:bg-gray-100 rounded-full">
                <X className="w-6 h-6 text-gray-600" />
              </Button>
            </div>
            {/* Add form fields here */}
            <p className="text-gray-600 mb-6">Form fields for adding a new medication would go here.</p>
            <div className="flex justify-end">
              <Button onClick={() => setShowAddModal(false)} className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-full shadow-md transition duration-300 ease-in-out transform hover:-translate-y-1">
                Add Medication
              </Button>
            </div>
          </div>
